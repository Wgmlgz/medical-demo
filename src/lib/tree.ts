...
export const init = async (canvas: HTMLCanvasElement, gui_container: HTMLElement) => {
  ...
  const volconfig = {
    ...
    renderstyle: 'iso',
    isothreshold: 0.15,
    colormap: 'viridis' as 'viridis' | 'gray'
  };
  const gui = new GUI({
  ...
  // Load the data ...
  const volume = await new Promise<Volume>((resolve, reject) => {
    new NRRDLoader().load(
  ...
  const texture = new THREE.Data3DTexture(
    volume.data as unknown as BufferSource,
    volume.xLength,
    volume.yLength,
    volume.zLength
  );
